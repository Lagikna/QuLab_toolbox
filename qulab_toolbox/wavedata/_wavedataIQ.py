...
class WavedataIQ(Wavedata):
    ...
    def timeFunc(self,kind='cubic'):  # 改进后支持复数
        ...
        _timeFuncI = self.I().timeFunc(kind=kind)
        _timeFuncQ = self.Q().timeFunc(kind=kind)
    ...
    def I(self):
        '''I波形 返回Wavedata类'''
        w = Wavedata(self.real, self.sRate)
        return w

    def Q(self):
        '''Q波形 返回Wavedata类'''
        w = Wavedata(self.imag, self.sRate)
        return w
