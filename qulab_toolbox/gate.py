...
#2.delay        :Move the Waveform 'delay' time(unit in us)
...
class xy_gauss():
    ...
    def __init__(self,para,delay,theta=0,alpha=0,samplerate=2.0e9):
        self.para=para
        self.delay=delay
        self.theta=theta
        self.alpha=alpha
    ...
    def _gate(self):
        ...
        (DC(1,self.para[0])<<self.para[0]))>>self.delay
...
class xy_pts(self,):
    ...
    def __init__(self,sqA,sqB,range=[0,50.0],samplerate=2.0e9):
        self.range=range
        self.samplerate=samplerate/1.0e6
        self._I=sqA
        self._Q=sqB
    ...
    def _pts(self):
        self._sqAI=self._I.set_range(self.range[0],self.range[1]).generateData(sampleRate=(self.samplerate))
        self._sqAQ=self._Q.set_range(self.range[0],self.range[1]).generateData(sampleRate=(self.samplerate))
